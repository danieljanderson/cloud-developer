...
describe('concat', () => {
  it('should add two strings into one', () => {
    const result = concat('hi', 'there');
    expect(result).to.equal('hithere');
  });
  it('should throw an error because the first string is empyt', () => {
    expect(() => {
      concat('', 'hi');
    }).to.throw('You need two strings');
  });
  it('should throw an error because the second string is empyt', () => {
    expect(() => {
      concat('hi', '');
    }).to.throw('You need two strings');
  });
